...
-- 成品图纸表
CREATE TABLE IF NOT EXISTS `doc_prod_drawing` (
    `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `pid` varchar(100) NOT NULL COMMENT '成品编号',
    `drawing_type` varchar(50) NOT NULL COMMENT '图纸类型',
    `item_name` varchar(200) DEFAULT NULL COMMENT '物品名称',
    `model` varchar(100) DEFAULT NULL COMMENT '型号',
    `dwg_file_id` bigint DEFAULT NULL COMMENT 'DWG文件ID',
    ...
    `deleted` int NOT NULL DEFAULT '0' COMMENT '逻辑删除标识（0：未删除，1：已删除）',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_pid_drawing_type` (`pid`, `drawing_type`),
    KEY `idx_pid` (`pid`),
    KEY `idx_dwg_file_id` (`dwg_file_id`),
    KEY `idx_pdf_file_id` (`pdf_file_id`),
    KEY `idx_status` (`status`),
    KEY `idx_deleted` (`deleted`),
    KEY `idx_create_time` (`create_time`),
    CONSTRAINT `fk_doc_prod_drawing_dwg_file` FOREIGN KEY (`dwg_file_id`) REFERENCES file_info (`id`),
    CONSTRAINT `fk_doc_prod_drawing_pdf_file` FOREIGN KEY (`pdf_file_id`) REFERENCES file_info (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='成品图纸表';

-- 零件图纸表
CREATE TABLE IF NOT EXISTS `doc_part_drawing` (
    `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `part_id` varchar(100) NOT NULL COMMENT '零件编号',
    `drawing_type` varchar(50) NOT NULL COMMENT '图纸类型',
    `item_name` varchar(200) DEFAULT NULL COMMENT '物品名称',
    `model` varchar(100) DEFAULT NULL COMMENT '型号',
    `dwg_file_id` bigint DEFAULT NULL COMMENT 'DWG文件ID',
    ...
    `deleted` int NOT NULL DEFAULT '0' COMMENT '逻辑删除标识（0：未删除，1：已删除）',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_part_id_drawing_type` (`part_id`, `drawing_type`),
    ...
    KEY `idx_deleted` (`deleted`),
    KEY `idx_create_time` (`create_time`),
    CONSTRAINT `fk_doc_part_drawing_dwg_file` FOREIGN KEY (`dwg_file_id`) REFERENCES file_info (`id`),
    CONSTRAINT `fk_doc_part_drawing_pdf_file` FOREIGN KEY (`pdf_file_id`) REFERENCES file_info (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='零件图纸表';