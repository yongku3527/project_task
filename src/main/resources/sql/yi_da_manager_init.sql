...
#     `originator_name` VARCHAR(100) DEFAULT NULL COMMENT '原实例发起人姓名',
#     `instance_create_time` VARCHAR(50) DEFAULT NULL COMMENT '原实例创建时间',
...
-- =====================================================
-- 表结构升级：为已存在的表添加发起人信息字段（如已存在则忽略）
-- =====================================================
ALTER TABLE `ding_db`.`yi_da_prcs_creation_log`
    ADD COLUMN  `originator_name` VARCHAR(100) DEFAULT NULL COMMENT '原实例发起人姓名' AFTER `error_msg`,
    ADD COLUMN  `instance_create_time` VARCHAR(50) DEFAULT NULL COMMENT '原实例创建时间' AFTER `originator_name`;
